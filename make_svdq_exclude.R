...
#ancestral
# delimitation_file <- "~/Desktop/Berkeley/Phylogenetic_analysis/SVDQ/Freund_population_ancestral_minus_hybrids.csv"
...
#sites
# delimitation_file <- "~/Desktop/Berkeley/Phylogenetic_analysis/SVDQ/Freund_population_sites_minus_hybrids.csv"
# outgroup_population <- c("Isoetes_bolanderii", "Isoetes_howellii")
...
#Individuals 
delimitation_file <- "~/Desktop/Berkeley/Phylogenetic_analysis/SVDQ/Freund_population_individuals_minus_hybrids.csv"
outgroup_population <- c("Isoetes_bolanderii", "Isoetes_howellii")
num_quartets <- 50000000
num_bootstrap_reps <- 1000
num_quartets_bootstrap <- 570000
...
samples_to_exclude <- c("FF290","CR5205")

# point to the dataset directory
dir <- "~/Desktop/Berkeley/Phylogenetic_analysis/BPP/Phased_run_files/Phased_data_excluded/"
# dir <- "Phased_data_nex/"
...
for(i in 1:length(phased_alignments)) {
  ...
  # remove samples that are to be excluded
  these_samples_to_exclude <- unlist(sapply(samples_to_exclude, function(x) grep(x, names(aln), value = TRUE)))
  aln <- aln[names(aln) %in% these_samples_to_exclude == FALSE]
...
# remove empty groups
tips_per_group <- tips_per_group[sapply(tips_per_group, is.null) == FALSE]
groups <- names(tips_per_group)
ngroups <- length(groups)
